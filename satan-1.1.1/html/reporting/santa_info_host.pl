...
<BODY>
<H1><IMG SRC=$HTML_ROOT/images/santa.gif> Results - $_host </H1>
<hr>
...
if (exists($hosttype{$_host})) {
	print CLIENT <<EOF;
	<li>Host type: <A HREF="santa_info_OStype.pl,$_type,">$hosttype{$_host}</A>
EOF
	$_exists = 1;
}
...
if (exists($server_info{$_host})) {
	for (sort split(/\n/, $server_info{$_host})) {
		...
		($_service = $_) =~ tr / \//?!/;
		print CLIENT <<EOF;
		<li> <A HREF="santa_info_servers.pl,$_service,"> $_ </A> server
EOF
	}
	$_exists = 1;
}
...
if (exists($client_info{$_host})) {
	for (sort split(/\n/, $client_info{$_host})) {
		...
		($_service = $_) =~ tr / \//?!/;
		print CLIENT <<EOF;
		<li> <A HREF="santa_info_clients.pl,$_service,"> $_ </A> client
EOF
	}
	$_exists = 1;
}
...
if (exists($all_hosts{$_host})) {
	...
	print CLIENT <<EOF;
	<li> Subnet <A HREF="santa_results_subnet.pl,$_subnet,"> $_subnet </A>
EOF
	$_exists = 1;
}
...
if (exists($total_trustee_count{$_host})) {
	...
	print CLIENT <<EOF;
	<li> $_count
	<A HREF="santa_results_trusting.pl,$_host,trustee_type,"> Trusting host(s) </a>
EOF
	$_exists = 1;
}
...
if (exists($total_trusted_count{$_host})) {
	...
	print CLIENT <<EOF;
	<li> $_count
	<A HREF="santa_results_trusted.pl,$_host,trusted_type,"> Trusted host(s) </a>
EOF
	$_exists = 1;
}
...
if ($_exists) {
	...
	<h3>Actions:</h3>
	<ul>
	<li> <A HREF="$HTML_SERVER/running/santa_run_form.pl,$_host,">Scan this host</a>
	</ul>
EOF
} else {
	print CLIENT <<EOF;
