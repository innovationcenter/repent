...
<BODY>
<H1><IMG SRC=$HTML_ROOT/images/santa.gif> Host Tables - by Class of Service </H1>
<hr>
EOF
...
if (sizeof(*servers) > 0) {
	...
	if ($server_counts{$_} > 0) {
	    ...
	    <a href="santa_info_servers.pl,$_service,"> $_</a>
...
if (sizeof(*clients) > 0) {
	...
	if ($client_counts{$_} > 0) {
	    print CLIENT <<EOF;
	    <dt><IMG SRC=$HTML_ROOT/dots/$_dot.gif ALT="$_alt"> 
	    <a href="santa_info_clients.pl,$_service,"> $_</a>
